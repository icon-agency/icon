...
page        →  page layers (canonical homepage system, news + work articles, work landing, hairline system, DS chrome)
```
...
- `colors.css` — **the only file where raw colour values (hex / rgb / oklch) may appear.** Raw values live in `:root` (light — the default on the canonical pages), `.dark`, and `.theme-blue`; shadcn neutral roles are mapped through `@theme inline` so a runtime theme class on `<html>` re-themes without a rebuild. Produces: semantic roles `--color-background/foreground/card/popover/primary/secondary/muted/accent/destructive/border/input/ring`, `--color-chart-1..5`, `--color-sidebar-*`; brand `--color-icon-black` (#0d0d0d — the footer's black, promoted) / `--color-icon-grey` / `--color-icon-blue`; fixed `--color-white/--color-black/--color-brand-blue/--color-shader-blue` (the last two both alias the one raw `--blue` and never re-theme). Also defines the non-`@theme` component roles (`--nav-bg-scrolled`, `--pager-surface`/`--pager-surface-strong`, `--work-article-text`/`--work-article-bg`), the fixed footer palette (`--footer-bg/-ink/-muted`), `--nav-bg-over-hero`, the scrim and shadow roles (`--scrim-soft/--scrim/--scrim-strong`, `--shadow-sm/--shadow-lg` — pure black at an alpha, fixed in every theme), and THE hairline `--rule` (`currentColor` at 25%, so one token draws on every ground — the old `--rule-on-color` escape hatch is gone) consumed by `utilities/rules.css`.
- `typography.css` — `--font-sans` (Kumbh Sans, Google) and `--font-serif` ("miller-text", Typekit kit `bhv7yrj`) via `@theme inline`; the fluid `--text-xs … --text-6xl` scale via `@theme` (wired to the `text-*` utilities; Tailwind's fixed 7xl–9xl are left at their defaults); plus the `:root` base `--heading-1`, the **voice roles** — `--voice-mid` (the site's one "sentence" size), `--voice-meta` (the quiet meta line), `--voice-label` (chrome and wayfinding), `--voice-headline` / `--voice-headline-lg` (article titles + the /news masthead), `--voice-display` (the big caps mastheads and the homepage intro statement, with its two line-height companions) and `--voice-accent` (the serif lockup line, ratio-locked to display) — one `clamp()` each; `--logo-h` is the wordmark's own fluid height, consumed by the sticky offsets — and `--font-weight-*`, `--line-height-*`, `--letter-spacing-*`, `--tracking-caps`.
- `spacing.css` — named `--spacing-3xs … --spacing-3xl` via `@theme` (generate `p-md`, `gap-lg`, `m-xl`, … alongside Tailwind's numeric scale); plus `:root` fluid rhythm `--space-section`, `--space-section-sm`, the fluid clamp ramp `--space-3xs … --space-2xl` (one spacing curve for the homepage's internal rhythm), THE 12-column `--grid-gutter` (a foundation since the /work landing adopted the homepage work pattern), and the LAYOUT/GUTTER foundations: `--container-max` (90rem), `--container-pad` (the capped container's padding), `--container-pad-wide` (THE wide gutter that holds the uncapped compositions; `--gutter-page` aliases it globally, and the homepage + /work landing re-point `--container-pad` to it for their u-containers), and `--gutter-page` (THE page gutter — an alias of the wide gutter since Aug 2026, one edge on every page — that the self-padded sections AND the fixed chrome — wordmark, mobile controls, footer padding — all read, so edges align by construction).
...
- `motion.css` — `--ease-standard/accelerate/decelerate` via `@theme` (exposed as `ease-*`); plus `:root` `--duration-fast/normal/slow/slower`, and the composed `--transition-base/--transition-slow`.
...
- `effects.css` — bespoke cross-component helpers: `.miller-text` (italic serif accent), `.masthead-accent` (upright serif accent line), `.invite-arrow` / `.invite-arrow--back` (the corner and back arrows), `.homepage-icon-black` (with its `.dark` override) and `.scrollbar-hide`. The v0 leftovers nothing consumed (`.fluid-heading`, `.text-card-*`, `.overlay-frosted-glass`, `.animated-grain`, …) were removed in Sep 2026.
...
Imported last so they cascade over everything. Seven files, each a deliberate scope:

- `home-page.css` — tiny layout shims for the home-A prototype.
- `work-article.css` — the work article (`work-article-master.html` + the client folios): the `.theme-work-article` **chameleon** (a per-client background + ink pair on `<html>` — see LESSONS.md — handed back to the site theme on scroll) plus the article frame, the layered-media anatomy the banner shares with the gallery figures, and the composition rules (which elements lean, how the stats band sits under a heading, the film's width in the reading column). The four blocks that used to live here — `work-gallery`, `work-stats`, `work-video`, `work-scroller` — were promoted to `src/components/` in Sep 2026 at their fifth consumer.
- `news-article.css` — the news article frame (`.news-article`): head, banner, share rail, Next up rail.
- `work-landing.css` — the /work landing's shims over the homepage work pattern (`.work-landing`).
...
Prefer adding a modifier to a component over a page-level override; a page layer earns a new rule only when the rule genuinely belongs to that page. A block that gains a second consumer moves to `src/components/` (how `news-card.css` and `quiet-link.css` happened).
...
6. **`npm run verify` is the gate.** `scripts/verify.mjs` rebuilds and byte-compares `css/main.css`, rejects raw colours outside `colors.css`, rejects media queries off the breakpoint table (a `verify-allow` comment with a reason is the escape hatch), and rejects literal inline styles. Run it before marking work done.
...
@import "./components/work-gallery.css";
@import "./components/work-stats.css";
@import "./components/work-video.css";
@import "./components/work-scroller.css";
