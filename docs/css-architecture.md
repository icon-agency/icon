...
Design values only. Every number, colour, font value, breakpoint, and motion constant in the system starts here. No selectors except `@theme`, `@theme inline`, and `:root` / `.dark`.

- `colors.css` — **the only file where raw colour values (hex / rgb / oklch) may appear.** Raw values live in `:root` (light) and `.dark` (dark — the site default); shadcn neutral roles are mapped through `@theme inline` so a runtime `.dark` class re-themes without a rebuild. Produces: semantic roles `--color-background/foreground/card/popover/primary/secondary/muted/accent/destructive/border/input/ring`, `--color-chart-1..5`, `--color-sidebar-*`; brand `--color-icon-black/--color-icon-grey/--color-icon-blue`; fixed `--color-white/--color-black/--color-shader-blue`. Also defines the non-`@theme` component roles `--nav-bg-scrolled`, `--work-article-text`, `--work-article-bg`.
- `typography.css` — `--font-sans` (Kumbh Sans, Google) and `--font-serif` ("miller-text", Typekit kit `bhv7yrj`) via `@theme inline`; the fluid `--text-xs … --text-9xl` scale via `@theme` (wired to the `text-*` utilities); plus `:root` semantic sizes (`--heading-1/-2/-3`, `--card-title`, `--card-subtitle`), `--font-weight-*`, `--line-height-*`, `--letter-spacing-*`.
- `spacing.css` — named `--spacing-3xs … --spacing-3xl` via `@theme` (generate `p-md`, `gap-lg`, `m-xl`, … alongside Tailwind's numeric scale); plus `:root` fluid rhythm `--space-section`, `--space-section-sm`, and layout `--container-max` (90rem), `--container-pad`.
...
- `reset.css` — additive on top of Tailwind's preflight: `text-size-adjust`, `body { overflow-x: hidden }`, block-level media defaults, the focus ring `:focus-visible { outline: 2px solid var(--color-icon-blue); outline-offset: 2px }`, and a reduced-motion `scroll-behavior` guard.
- `typography.css` — body font/size/colour (Kumbh Sans @ `--text-base`, with a colour/background transition for the on-scroll theme flip), `h1` weight + `--heading-1` size, `text-wrap: balance` on `h1`–`h4`, `text-wrap: pretty` on `p`/`li`/`dd`. These wrap rules are a base-layer concern — never re-applied per component.
...
- `animations.css` — the `[data-animate]` scroll-reveal host (gated by `.js-animations`, toggled with `.is-visible`, no-JS-safe), the `grain`/`scroll`/`fadeIn`/`slideIn` keyframes, and reduced-motion guards (`[data-animate]` + `[data-parallax*]`). See [animation.md](animation.md). The `[data-animate]` host is an available primitive, not the active system.
...
One BEM block per file. Each file owns everything that starts with that block name. Variants live in the same file unless they grow large enough to justify splitting. Current files: `site-header.css`, `hero.css`, `logo-loop.css`, `tagline.css` (imported in that order).
...
Page-specific tweaks live in `src/utilities/home-page.css` (`.home-main`, `.home-intro`, `.home-placeholder`) — the one place page-level CSS is allowed, imported last so it cascades over everything. Prefer adding a modifier to a component over a page-level override.
