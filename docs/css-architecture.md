...
- `spacing.css` — named `--spacing-3xs … --spacing-3xl` via `@theme` (generate `p-md`, `gap-lg`, `m-xl`, … alongside Tailwind's numeric scale); plus `:root` fluid rhythm `--space-section`, `--space-section-sm`, the fluid clamp ramp `--space-3xs … --space-2xl` (one spacing curve for the homepage's internal rhythm), THE 12-column `--grid-gutter` (a foundation since the /work landing adopted the homepage work pattern), and the LAYOUT/GUTTER foundations: `--container-max` (90rem), `--container-pad` (the capped container's padding), `--container-pad-wide` (THE wide gutter that holds the uncapped compositions — the homepage AND the /work landing re-point `--container-pad` and, at body level, `--gutter-page` to it), and `--gutter-page` (the full-bleed page gutter that the self-padded sections AND the fixed chrome — wordmark, mobile controls, footer padding — all read, so edges align by construction).
