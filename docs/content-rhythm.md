...
- Three **voice roles** (in `src/theme/typography.css`) carry most of the canonical pages' running text — reach for these before inventing a size:
  ...
  - the **headline voice** (`--voice-headline`) — the register a step under display, above the prose h1: the article titles and the /news `h1` masthead share `clamp(2.25rem, 3.6vw, 6rem)` (header B's stacked title is this register at 1.5×).
