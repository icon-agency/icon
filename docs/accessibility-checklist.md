...
- Contrast meets WCAG AA. The palette is oklch neutral semantic roles plus brand `--icon-black` / `--icon-grey` / `--icon-blue`, defined only in `src/theme/colors.css`. `homeC` and the DS index default to the **light** palette; the news pages OPEN dark and hand over to light mid-scroll (`js/theme-handover.js`), so their pairings are genuinely exercised in both themes on every visit — and `.dark` / `.theme-blue` stay inspectable everywhere via the dev toggle (the prototypes still open dark). Verify any new colour pairing under **all three** themes: `--icon-black` inverts in dark mode, and the blue theme swaps ink/surface entirely.
