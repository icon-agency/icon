...
- **Content links keep their real Drupal paths and do NOT resolve statically.**
  ...
  being honoured server-side by a Views exposed filter.- **The news stories are mirrored from iconagency.com.au**, not invented:  ( reads a downloaded story page into title, date, category, tile, banner and body blocks;  creates or updates the News item by title, then alias, with the old site's path as the alias, the original files behind the old site's image styles as Image media, a hosted mp4 as a Work video paragraph — the news body allows  for this — and a YouTube film as a News article video). The latest fourteen are in; the old site has sixty-odd more, same steps. The invented sample stories from  are unpublished, not deleted.
