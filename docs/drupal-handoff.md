...
- **The hero is an SDC too, fed by the block** — `hero.component.yml` declares `libraryOverrides.dependencies` on `icon/hero-loader` (footer, weighted ahead of the externals) and `icon/home-c` (GSAP, SplitText, DrawSVG, Lenis), so the whole homepage system attaches whenever the hero renders. Its `pieces` prop is an array of objects, which Canvas has no shape for, so the SDC is rejected from Canvas's library — by design: editors place the Homepage hero block, never the raw hero. Slides carry `data-client-url`, and `js/home-c.js` renders the client name as `<a class="hero__client-link">` when one is present (the static reel has none). Same for `work-gallery` / `work-stats` / `work-scroller` (Paragraphs-fed).
