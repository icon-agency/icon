...
- **The feeds are Views blocks, which Canvas accepts as components.** `news.latest` (→ the news rail) and `clients.marquee` (every published **Logo** media item, by Order then name → the `clients` SDC's two rows).
- **The hero and Featured work are CHOSEN on the homepage.** Two `icon_site` blocks with fully-validatable settings, so Canvas edits them in its side panel. `Featured work` (`block.icon_featured_work`, on `WorkPicksBlockBase`) has five project pickers (autocomplete by title, in row order; empty picks fall back to the latest) and renders through the work View's `featured` display, passing the picks as its `nid` argument (`nid+nid+…`, `break_phrase`; the preprocess keeps the picked order). `Homepage hero` (`block.icon_hero`, `HeroBlock.php`) is FREE content, independent of the Work items: up to four slides, each a media item (Video MP4 or Image, picked by name from the media library — editors upload under Media first), the client name shown bottom-left, and a link for that name (the grow-line hover). The form carries the reference note: films must be 6 seconds, muted, loop (the reel plays each slide 3s with a cross-fade; nothing checks the length). The block renders straight into the `icon:hero` SDC via `icon_site_media_source()` (image style or video file → `{type, src, alt, width, height}`; the theme's `_icon_media_source()` delegates to it).
...
- **Client logos are a `logo` media type** (file source, `svg png`, the name is the alt, an Order field), per `assets/client-logos/README.md` — editors add a client under Media → Add → Logo, and the marquee block picks it up anywhere it is placed. `scripts/clients-sample-content.php` seeds the eighteen.
...
- **The hero is an SDC too, fed by the block** — `hero.component.yml` declares `libraryOverrides.dependencies` on `icon/hero-loader` (head) and `icon/home-c` (GSAP, SplitText, DrawSVG, Lenis), so the whole homepage system attaches whenever the hero renders. Its `pieces` prop is an array of objects, which Canvas has no shape for, so the SDC is rejected from Canvas's library — by design: editors place the Homepage hero block, never the raw hero. Slides carry `data-client-url`, and `js/home-c.js` renders the client name as `<a class="hero__client-link">` when one is present (the static reel has none). Same for `work-gallery` / `work-stats` / `work-scroller` (Paragraphs-fed).
