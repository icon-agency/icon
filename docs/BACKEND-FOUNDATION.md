...
| 10 | **Local tooling** — a Lando file beside the DDEV one, if the team standard is Lando | Both work; the choice should follow the developers, not the repo | Half a day | Versions (PHP, MariaDB, Redis, Solr) must change in hosting config and local config together — document it as Frank's README does |
| 11 | **Hosting push / pull** — the Platform.sh push and pull of database and files as project commands | Frank's `lando pull` / `lando push`; the DDEV equivalent is a pair of scripts around the CLI we already use | Half a day | After the project exists |
...
| 2 | Client on a Work item | A text field | A **Client** vocabulary if clients will ever be listed, filtered or joined to their logo; keep text only if it is deliberately a byline | Half a day | Migration + relinking | An agent can fill or check a reference; not a string |
