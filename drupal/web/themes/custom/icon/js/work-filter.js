/* work-filter.js — GENERATED by scripts/theme-js.mjs from the repo-root js/work-filter.js.
 * Do not edit here: edit the source and run `npm run build:theme`.
 * The source IIFE runs once per page inside Drupal.behaviors.iconWorkFilter.attach()
 * (once() on <html>), so the behaviour body is byte-identical to the prototype's. */
((Drupal, once) => {
  Drupal.behaviors.iconWorkFilter = {
    attach(context) {
      if (!once("work-filter", "html", context).length) return;
