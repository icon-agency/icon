...
/**
 * Environment layers, in this order:
 *
 * 1. settings.platform.php — the hosting platform's database, paths, salt
 *    (present when PLATFORM_PROJECT is set; added with the hosting config).
 * 2. settings.ddev.php — generated by DDEV for the local site (untracked).
 * 3. This file's environment switch: which config split is on and what the
 *    environment indicator says. Local turns the `local` split on (devel,
 *    stage_file_proxy) and the dev conveniences; every other environment
 *    leaves it off.
 * 4. settings.local.php — uncommitted, overrides everything (below).
 */
if (getenv('PLATFORM_PROJECT') && file_exists(__DIR__ . '/settings.platform.php')) {
...
if (getenv('IS_DDEV_PROJECT') == 'true') {
  ...
  $config['environment_indicator.indicator']['bg_color'] = '#2f6f2f';
  $config['environment_indicator.indicator']['fg_color'] = '#ffffff';
  // read the built CSS and JS as files, not aggregates, while developing
